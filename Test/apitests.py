...
# 8. ANÁLISIS — GET
# ─────────────────────────────────────────────────────────────────────────────
class AnalisisGetTests(TestCase):
    ...
    def setUp(self):
        ...
        self.proyecto = crear_proyecto(self.user)
        self.analisis = crear_analisis(self.proyecto, score=85)

        proyecto_ajeno = crear_proyecto(self.otro, 'Ajeno')
        self.analisis_ajeno = crear_analisis(proyecto_ajeno, score=50)

    def test_get_lista_analisis_propios(self):
        """GET /api/analisis/ devuelve solo análisis del usuario autenticado."""
        resp = self.client.get(reverse('api_analisis_list'))
        self.assertEqual(resp.status_code, 200)
        ids = [a['id'] for a in resp.data['analisis']]
        self.assertIn(self.analisis.pk, ids)
        self.assertNotIn(self.analisis_ajeno.pk, ids)

    def test_get_detalle_analisis_propio(self):
        """GET /api/analisis/<pk>/ devuelve el análisis con vulnerabilidades."""
    ...
    def test_get_detalle_analisis_ajeno_devuelve_404(self):
        """GET /api/analisis/<pk>/ de análisis ajeno devuelve 404."""
        resp = self.client.get(
            reverse('api_analisis_detail', args=[self.analisis_ajeno.pk])
        )
        self.assertEqual(resp.status_code, 404)
...
# 9. ANÁLISIS — DELETE
# ─────────────────────────────────────────────────────────────────────────────
class AnalisisDeleteTests(TestCase):
    ...
    def setUp(self):
        ...
        self.proyecto = crear_proyecto(self.user)
        self.analisis = crear_analisis(self.proyecto)

        proyecto_ajeno = crear_proyecto(self.otro, 'Ajeno')
        self.analisis_ajeno = crear_analisis(proyecto_ajeno)

    def test_delete_analisis_propio(self):
        """DELETE /api/analisis/<pk>/ elimina el análisis del usuario."""
        pk = self.analisis.pk
        resp = self.client.delete(reverse('api_analisis_detail', args=[pk]))
        self.assertEqual(resp.status_code, 200)
        self.assertFalse(Analisis.objects.filter(pk=pk).exists())

    def test_delete_analisis_ajeno_devuelve_404(self):
        """DELETE de análisis ajeno devuelve 404 y no lo elimina."""
        pk = self.analisis_ajeno.pk
        resp = self.client.delete(reverse('api_analisis_detail', args=[pk]))
        self.assertEqual(resp.status_code, 404)
        self.assertTrue(Analisis.objects.filter(pk=pk).exists())

    def test_delete_analisis_no_elimina_el_proyecto(self):
        """DELETE de un análisis no elimina el proyecto asociado."""
        pk_proyecto = self.proyecto.pk
        self.client.delete(
            reverse('api_analisis_detail', args=[self.analisis.pk])
        )
        self.assertTrue(Project.objects.filter(pk=pk_proyecto).exists())